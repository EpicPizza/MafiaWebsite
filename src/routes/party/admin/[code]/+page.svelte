...
<div class="bg-zinc-900 px-6 py-4 w-full flex items-center justify-between">
    <div class="flex gap-3 items-center">
        ...
        {#if party && party.state != 425}
            <button
                disabled={data.state == 312}
                on:click={() => {
                    endGame();
                }}
                class="disabled:opacity-30 disabled:cursor-not-allowed bg-red-400 w-20 text-black px-3 py-2 rounded-lg font-bold text-lg"
            >
                End
            </button>
        {:else}
            <button
                ...
                class="disabled:opacity-30 disabled:cursor-not-allowed bg-green-400 w-20 text-black px-3 py-2 rounded-lg font-bold text-lg"
            >
                Start
            </button>
        {/if}
    ...
    {#if party && (party.state == 345 || party.state == 384 || party.state == 385 || party.state == 390)}
        <div class="flex gap-3 items-center">
            <button
                ...
                class="bg-zinc-400 disabled:opacity-30 disabled:cursor-not-allowed text-black px-3 py-2 rounded-lg font-bold text-lg"
            >
                Locked
            </button>
            <button
    ...
    <div class="flex gap-3 items-center">
        <button
            ...
            class="bg-zinc-400 text-black px-3 py-2 rounded-lg font-bold text-lg"
        >
...
{#if showPlayers && party}
    <div
        class="bg-zinc-700 px-7 py-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 grid gap-2"
    >
        {#each party.players as player}
            <div class="flex items-center gap-2">
                <p>{player.displayName || player.id}</p>
                <button
                    on:click={() => {
...
<div class="px-6 py-4 mb-20">
    {#if data.state == 312 && randomizing == false && party}
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center flex-col gap-1">
                            ...
                            <input
                                ...
                                class="px-1.5 py-0.5 w-14 text-sm rounded-md bg-zinc-800"
                            />
                        </div>
                    </div>
                    <textarea
                        bind:value={role.description}
                        class="w-full mt-3 px-3 py-2 rounded-md bg-zinc-800"
                    ></textarea>
                    <div class="flex items-center justify-between">
                        <button
                            on:click={() => {
                            ...
                            class="bg-red-600 mt-3 w-20 text-white px-3 py-2 rounded-lg font-bold text-lg block"
                        >
                            Delete
                        </button>
        ...
        <button
            ...
            class="bg-yellow-400 w-20 text-black px-3 py-2 rounded-lg font-bold text-lg mt-4 block"
        >
            Create
        </button>
        ...
        <button
            ...
            class="bg-red-600 mt-20 w-20 text-white px-3 py-2 rounded-lg font-bold text-lg block"
        >
            Next
        </button>
    {:else if data.state == 312 && randomizing && party}
        <div class="grid-cols-1 md:grid-cols-2 lg:grid-cols-3 grid gap-3">
            {#key data.encrypted.roles}
                {#each party.players as player}
                    ...
                    <div
                        class="px-4 py-3 rounded-lg h-44 bg-zinc-900 w-full flex items-center justify-around"
                    >
        ...
        <button
            disabled={countRoles(creator) != party.players.length}
            on:click={() => {
                randomize();
            }}
            class="disabled:opacity-30 disabled:cursor-not-allowed bg-yellow-400 text-black px-3 py-2 rounded-lg font-bold text-lg mt-4 block"
        >
            Randomize
        </button>
        {#if countRoles(creator) != party.players.length}
            <p class="opacity-80 italic mt-4">
                Roles cannot be randomized since the number of roles do not
                match the number of players. ({countRoles(creator)} Roles, {party
        ...
        <div class="flex items-center gap-3">
            <button
                ...
                class="bg-zinc-400 mt-20 w-20 text-black px-3 py-2 rounded-lg font-bold text-lg block"
            >
                Back
            </button>
            <button
                ...
                class="disabled:opacity-30 disabled:cursor-not-allowed bg-green-400 mt-20 w-20 text-black px-3 py-2 rounded-lg font-bold text-lg block"
            >
                Next
            </button>
        </div>
            ...
            {#each data.encrypted.roles as player}
                ...
                >
                    <div class="text-center w-full">
                        <p class="text-3xl font-bold mb-3">
                            {player.displayName || player.id} - {player
                                ? player.name
                                : "?"} ({player.type})
                        </p>
                        <textarea
                            ...
                            class="px-3 py-2 mt-4 bg-zinc-800 rounded-md w-full min-h-12"
                        />
                    </div>
                </div>
            {/each}
        ...
        <div class="flex items-center gap-3 mt-8">
            <button
                on:click={() => {
                    ...
                    data.encrypted.roles = data.encrypted.roles;
                }}
                class="bg-zinc-400 w-20 text-black px-3 py-2 rounded-lg font-bold text-lg block"
            >
                Reset
            </button>
            <button
                ...
                class="disabled:opacity-30 disabled:cursor-not-allowed bg-green-400 w-20 text-black px-3 py-2 rounded-lg font-bold text-lg block"
            >
                Send
            </button>
        </div>
